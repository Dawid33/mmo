    ...
    quote! {
            ...
            T: Default + serde::Serialize + for<'a> serde::Deserialize<'a>,
        {
            ...
            pub fn as_mut(&mut self, undo: Box<dyn Fn(&mut T)>) -> &mut T {
                let order = self.info.order.fetch_add(1, ::std::sync::atomic::Ordering::SeqCst);
                let trans = self.info.order.load(::std::sync::atomic::Ordering::SeqCst);
                self.log
                    .lock()
                    .unwrap()
        ...
        #[derive(Default, ::rollback::Debug, ::rollback::serde::Serialize, ::rollback::serde::Deserialize, ::rollback::RollbackDerive)]
        #ast
