...
use crate::rapier::math::{AngVector, Isometry, Point, Real, Rotation, Vector};
use crate::rapier::utils::SimdCross;
use num::Zero;
...
impl RigidBody {
    ...
    pub(crate) fn predict_position_using_velocity_and_forces_with_max_dist(
        ...
        let clamped_linvel = linvel_norm.min(max_dist * crate::rapier::utils::inv(dt));
        let clamped_dt = dt * clamped_linvel * crate::rapier::utils::inv(linvel_norm);
        new_vels.integrate(
            clamped_dt,
            &self.pos.position,
            &self.mprops.local_mprops.local_com,
