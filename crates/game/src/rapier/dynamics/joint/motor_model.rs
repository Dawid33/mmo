    ...
    pub fn combine_coefficients(
        ...
        match self {
            MotorModel::AccelerationBased => {
                let erp_inv_dt = stiffness * crate::rapier::utils::inv(dt * stiffness + damping);
                let cfm_coeff = crate::rapier::utils::inv(dt * dt * stiffness + dt * damping);
                (erp_inv_dt, cfm_coeff, Real::from(0.0))
            }
            MotorModel::ForceBased => {
                let erp_inv_dt = stiffness * crate::rapier::utils::inv(dt * stiffness + damping);
                let cfm_gain = crate::rapier::utils::inv(dt * dt * stiffness + dt * damping);
                (erp_inv_dt, Real::from(0.0), cfm_gain)
            }
        }
    }
}
