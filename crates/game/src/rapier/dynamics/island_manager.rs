...
use crate::rapier::geometry::{ColliderSet, NarrowPhase};
...
///
/// You rarely interact with this directly - it's automatically managed by [`PhysicsPipeline`](crate::rapier::pipeline::PhysicsPipeline).
    ...
    pub(crate) fn update_active_set_with_contacts(
                ...
                for inter in narrow_phase.contact_pairs_with(*collider_handle) {
                    for manifold in &inter.manifolds {
                        if !manifold.data.solver_contacts.is_empty() {
                            let other = crate::rapier::utils::select_other(
        ...
        while let Some(handle) = self.stack.pop() {
            ...
            for inter in impulse_joints.attached_enabled_joints(handle) {
                let other = crate::rapier::utils::select_other((inter.0, inter.1), handle);
