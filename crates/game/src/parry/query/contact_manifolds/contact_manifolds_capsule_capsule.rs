...
use crate::parry::query::{ContactManifold, TrackedContact};
#[cfg(feature = "dim2")]
use crate::parry::shape::SegmentPointLocation;
use crate::parry::shape::{Capsule, PackedFeatureId, Shape};
    ...
    ContactData: Default + Copy,
{
    ...
    let (loc1, loc2) = crate::parry::query::details::closest_points_segment_segment_with_locations_nD(
        (&seg1.a, &seg1.b),
        (&seg2_1.a, &seg2_1.b),
    );
    ...
    if let (Some(dir1), Some(dir2)) = (seg1.direction(), seg2_1.direction()) {
        if dir1.dot(&dir2).abs() >= crate::parry::utils::COS_FRAC_PI_8
            && dir1.dot(&local_n1).abs() < crate::parry::utils::SIN_FRAC_PI_8
        {
            // Capsules axes are almost parallel and are almost perpendicular to the normal.
            // Find a second contact point.
            if let Some((clip_a, clip_b)) = crate::parry::query::details::clip_segment_segment_with_normal(
                (seg1.a, seg1.b),
                (seg2_1.a, seg2_1.b),
                *local_n1,
            ) {
    ...
    ContactData: Default + Copy,
{
    ...
    let seg2_1 = capsule2.segment.transformed(pos12);
    let (loc1, loc2) =
        crate::parry::query::closest_points::closest_points_segment_segment_with_locations_nD(
            (&seg1.a, &seg1.b),
            (&seg2_1.a, &seg2_1.b),
        );
