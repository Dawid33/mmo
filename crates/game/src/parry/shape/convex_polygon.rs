...
use crate::parry::shape::{FeatureId, PackedFeatureId, PolygonalFeature, PolygonalFeatureMap, SupportMap};
...
impl ConvexPolygon {
    ...
    pub fn from_convex_hull(points: &[Point<Real>]) -> Option<Self> {
        let vertices = crate::parry::transformation::convex_hull(points);
    ...
    pub fn from_convex_polyline(mut points: Vec<Point<Real>>) -> Option<Self> {
        ...
        let eps = ComplexField::sqrt(crate::parry::math::DEFAULT_EPSILON);
...
impl PolygonalFeatureMap for ConvexPolygon {
    fn local_support_feature(&self, dir: &Unit<Vector<Real>>, out_feature: &mut PolygonalFeature) {
        let cuboid = crate::parry::shape::Cuboid::new(self.points[2].coords);
