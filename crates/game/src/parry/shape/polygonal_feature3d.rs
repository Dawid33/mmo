...
use crate::parry::shape::{PackedFeatureId, Segment, Triangle};
use crate::parry::utils::WBasis;
use na::Point2;
...
impl PolygonalFeature {
    ...
    fn contacts_edge_edge<ManifoldData, ContactData: Default + Copy>(
        ...
        if let (Some(tangent1), Some(tangent2)) = (tangent1, tangent2) {
            let parallel = tangent1.dot(&tangent2) >= crate::parry::utils::COS_FRAC_PI_8;

            if !parallel {
                let seg1 = (&projected_edge1[0], &projected_edge1[1]);
                let seg2 = (&projected_edge2[0], &projected_edge2[1]);
                let (loc1, loc2) =
                    crate::parry::query::details::closest_points_segment_segment_with_locations_nD(
                        seg1, seg2,
                    );
        ...
        //clipping in 2D?
        if let Some(clips) = crate::parry::query::details::clip_segment_segment(
