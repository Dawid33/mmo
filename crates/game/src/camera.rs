...
impl Controller for CameraController {
    fn on_tick<'a>(&mut self, data: &mut Undo<GameData>) {
        ...
        for (p, e_id) in data.players.iter() {
            ...
            if p.bevy.key_held(&bevy::input::keyboard::KeyCode::KeyW) {
                linvel.z = -0.1 * SPEED
            }
            if p.bevy.key_held(&bevy::input::keyboard::KeyCode::KeyS) {
                linvel.z = 0.1 * SPEED
            }
            if p.bevy.key_held(&bevy::input::keyboard::KeyCode::KeyA) {
                linvel.x = -0.1 * SPEED
            }
            if p.bevy.key_held(&bevy::input::keyboard::KeyCode::KeyD) {
                linvel.x = 0.1 * SPEED
            }
            let mut linvel = rotation.transform_vector(&linvel);

            if p.bevy.key_held(&bevy::input::keyboard::KeyCode::Space) {
                linvel.y = 0.1 * SPEED
            }
            if p.bevy
                .key_held(&bevy::input::keyboard::KeyCode::ControlLeft)
            {
                linvel.y = -0.1 * SPEED
            }
