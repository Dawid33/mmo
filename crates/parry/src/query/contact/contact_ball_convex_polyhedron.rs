...
pub fn contact_convex_polyhedron_ball(
    ...
    if let Some((dir1, len)) =
        Unit::try_new_and_get(proj.point - center2_1, Real::from(crate::math::DEFAULT_EPSILON))
    {
        if proj.is_inside {
        ...
        normal1 = shape1
            ...
            .or_else(|| Unit::try_new(proj.point.coords, Real::from(crate::math::DEFAULT_EPSILON)))
