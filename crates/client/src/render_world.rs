...
use game::{
    ChunkMesh, EntityId, EntityKey, GameData, GameDataUpdate, IsometryReal, RegionId, ASPECT,
};
use log::info;
use rapier3d::na::{Matrix4, Perspective3};
use wgpu::{
    util::{BufferInitDescriptor, DeviceExt},
    BindGroup, BufferUsages, Device, Queue, RenderPass,
};
use winit::{
...
use crate::layout::CAMERA_LAYOUT_DESC;
...
impl TrueWorld {
    pub fn new(data: &GameData, reciever: Receiver<GameDataUpdate>) -> Self {
        ...
        for (id, _) in data.ecs.empty.iter() {
            ...
            let mesh = if let Some(mesh) = data.ecs.mesh.try_get(id) {
                Some(ChunkMesh::new(&mesh))
            } else {
                None
            };
...
pub struct GpuEntity {
    ...
    position: Option<IsometryReal>,
    voxel_mesh: Option<(wgpu::Buffer, usize)>,
}
...
impl RenderWorld {
        ...
        for (i, region) in self.regions.iter_mut() {
            while let Ok(event) = region.reciever.try_recv() {
                match event.update_kind {
                    ...
                    game::GameDataUpdateKind::SetVoxelMesh(entity_key, mesh) => {
                        let e = region.entities.get_mut(&entity_key).unwrap();
                        e.voxel_mesh = mesh;
                        self.lerp_set.get_mut(i).unwrap().insert(entity_key);
                    }
                    game::GameDataUpdateKind::SetFreeCam(mode) => {
                        if mode {
    ...
    pub fn draw(&self, mut renderpass: RenderPass<'_>) {
        ...
        for (i, region) in self.data.entities.iter() {
            for (_, e) in region {
                if let Some((buf, size)) = &e.voxel_mesh {
